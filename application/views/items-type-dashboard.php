...
<div class="item-type-dashboard-page asset-type-dashboard-page">
<div class="asset-dashboard-toolbar row mb-4">
    ...
    <div class="col-md-6">
        <div class="form-inline float-right">
            <label class="mr-2" for="searchInput">Search Items:</label>
            <input type="text" class="form-control form-control-sm" id="searchInput" placeholder="Enter item type">
        </div>
    </div>
</div>

<div class="asset-dashboard-grid row" id="item-card-container">
    <?php foreach ($this->steve->items_type_dashboard() as $t): ?>
        <?php // Legacy direct status keys were replaced with null-coalescing fallbacks because missing statuses caused warnings. ?>`r`n        <div class="col-md-3 mb-4 item-card" data-item-type="<?= strtolower($t['item_type']); ?>" data-status-serviceable="<?= (($t['statuses']['SERVICEABLE'] ?? 0) > 0) ? 'true' : 'false' ?>" data-status-unserviceable="<?= (($t['statuses']['UNSERVICEABLE'] ?? 0) > 0) ? 'true' : 'false' ?>" data-status-maintenance="<?= (($t['statuses']['MAINTENANCE'] ?? 0) > 0) ? 'true' : 'false' ?>">
       
           <div class="custom-card shadow">
                <?php
    $itemPicture = $t['item_picture'] ?? '';
    $itemTypeId = $t['item_type_id'];
    $relativePath = 'storage/ItemType-' . $itemTypeId . '/' . $itemPicture;
    $absolutePath = FCPATH . $relativePath;

    $isThumbnail = empty($itemPicture) || !file_exists($absolutePath);
...
<div class="asset-card-icon"><i class="fas fa-cogs"></i></div>
<form method="post"
      action="<?= site_url("items_type_dashboard/item_type_picture"); ?>"
      class="dropzone <?= $backgroundClass ?>"
      style="background-image: url('<?= $backgroundUrl ?>');">
    <?php // New dashboard UI: explicit upload pill retained so component cards match the asset dashboard reference. ?>
    ...
    <input type="hidden" name="id" readonly value="<?= $itemTypeId; ?>">
    <div class="fallback">
        <input name="file" type="file" accept="image/*" />
                ...
                <a href="<?= site_url('items?type_filter=' . urlencode($t['item_type_id'])) ?>" class="d-block text-decoration-none">
                <div class="card-body text-center">
                    <h5 class="card-title main-title"><?= $t['item_type']; ?></h5>
                    <!-- Legacy list layout replaced with 2x2 metric boxes for dashboard consistency.
