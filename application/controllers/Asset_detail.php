...
class Asset_detail extends CI_Controller
{
        ...
        if ($this->input->get('type')) {
            ...
            if (!isSuperAdmin()) {
                ...
                if ($type == 'In_Use') {
                    ...
                    $query1 = "SELECT orders.order_id, asset_types.name, equipments_asset.equipment_name, 
                    ...
                    LEFT JOIN (SELECT qc.*,@row_number := IF(@prev_value = qc.reg_no, @row_number + 1, 1) AS rn,@prev_value := qc.reg_no AS prev_value FROM (
                    SELECT * FROM order_equipment_bin_qr_codes WHERE scanned = 1 AND reg_no IS NOT NULL ORDER BY reg_no, created_at DESC) qc CROSS JOIN (SELECT @row_number := 0, @prev_value := NULL) AS vars)
                    ...
                    AND equipments_asset.branch_office_id IN (" . implode(',', getUserActiveBranchsId()) . ')';

                    $assets = $this->db->query($query1)->result();

                    $assets_deployed = $this->db->where_in('branch_office_id', getUserActiveBranchsId())->where('equipment_status', 'In use')->count_all_results('equipments_asset');
                    ...
                    $assets = $this->db->select('asset_types.name, equipments_asset.equipment_name, equipments_asset.equipment_registration, equipments_asset.equipment_status')
                        ...
                        ->where_in('branch_office_id', getUserActiveBranchsId())
                        ->get()
                        ->result();
                ...
                if ($type == 'In_Use') {
                    ...
                    // this query is use for UER website
                    ...
                    $assets = $this->db->select('equipments_asset.*, 
            ...
            ->join('states', 'states.id = locations.state_id', 'left')
                        ...
                        ->where('equipments_asset.faulty_type_id = ', null)
                        ->get()
                        ->result();


                    $assets_deployed = $this->db->where('equipment_status', 'In use')->where('faulty_type_id =', null)->count_all_results('equipments_asset');
    ...
    public function assetDeployedLocation()
    {
        ...
        //  uncomment this query when live on server
        // $query1 = "WITH RankedData AS (
        //     SELECT qc.*, ROW_NUMBER() OVER (PARTITION BY qc.reg_no ORDER BY qc.created_at DESC) AS rn
        //     FROM order_equipment_bin_qr_codes qc
        //     WHERE qc.scanned = 1 AND qc.reg_no IS NOT NULL
        ...
        // comment this query when live on server
        $query1 = "SELECT 
        company_addresses.*,
    ...
    FROM 
        ...
        LEFT JOIN (
            SELECT 
                qc.*,
                IF(@prev_reg_no = qc.reg_no, @rn := @rn + 1, @rn := 1) AS rn,
            ...
            FROM 
                ...
                (SELECT @rn := 0, @prev_reg_no := NULL) vars
        ) AS RankedData ON RankedData.order_id = orders.order_id
        LEFT JOIN asset_types ON asset_types.asset_id = RankedData.asset_type_id
        LEFT JOIN equipments_asset ON equipments_asset.equipment_registration = RankedData.reg_no
    WHERE 
        orders.second_order_type IS NULL 
        AND RankedData.rn = 1 
        AND equipments_asset.equipment_status = 'In use'
    ";
