...
$(document).ready(function () {
    ...
    $.ajax({
        // Legacy root URL breaks when app runs in /assets_IT-usman/: url: '/item_dashboard/itemLocationPointer'
        url: amsUrl('item_dashboard/itemLocationPointer'), // Replace with your actual endpoint
        method: 'GET',
            ...
            if (Array.isArray(response.states)) {
                response.states.forEach(asset => {
                    ...
                    const marker = L.marker([asset.latitude, asset.longitude], { icon: icon })
                        .addTo(map)
                        .bindPopup(`
                            <strong>Item Type:</strong> ${asset.item_type}<br>
                            <strong>Item Name:</strong> ${asset.item_name}<br>
